...
export interface GameState {
  ...
  zobristHash: bigint;       // 增量 Zobrist 哈希
}
...
export interface AIMoveDecision {
  ...
  debugInfo?: AIMoveDebugInfo; // 用于教学展示
}

export type AIMoveDebugInfo = {
  engine?: string;
  strategy?: string;
  mode?: string;
  reason?: string;
  ...
  reuseTtl?: number;
  workerCount?: number;
  mctsVisitTarget?: number;
  pvsNodeTarget?: number;
  tacticalKind?: string;
  tacticalMoves?: number;
  tacticalNodes?: number;
  tacticalDepth?: number;
  tacticalLine?: number;
  tacticalDefenseKind?: string;
  tacticalDefenseMoves?: number;
  tacticalDefenseChecks?: number;
  usedVCDT?: boolean;
  multithreading?: 'requested_but_unsupported' | 'enabled';
  [key: string]: unknown;
};
