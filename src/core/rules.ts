...
import { ZOBRIST_TABLE, toggleSideHash } from './zobrist';
...
function hasSixFrom(
  ...
  for (const { dx, dy } of DIRS) {
    ...
    let nx = x + dx;
    let ny = y + dy;
    while (isInside(nx, ny) && board[ny][nx] === value) {
      count += 1;
      nx += dx;
      ny += dy;
    }
...
export function checkWinnerFromMove(
  ...
  for (const pos of move.positions) {
    ...
    if (hasSixFrom(state.board, pos.x, pos.y, value)) {
      return move.player;
    }
  }
  return undefined;
...
export function applyMoveWithWinner(
  ...
  for (const pos of move.positions) {
    ...
    if (seen.has(key)) {
      throw new Error(`Duplicate position in move: (${x}, ${y})`);
    }
    seen.add(key);
  ...
  // 6. 判断胜负
  const winner = checkWinnerFromMove(next, move);
  if (winner) {
    next.winner = winner;
...
export function tryApplyMoveWithWinner(
  state: GameState,
  move: Move,
  opts?: { allowIncomplete?: boolean },
): ApplyMoveResult {
  try {
    return { ok: true, state: applyMoveWithWinner(state, move, opts) };
  } catch (err) {
